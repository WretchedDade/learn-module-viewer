...
export function MarkdownContent({ content, title = "View Content", previewLength = 500, showFullContent = false, expandedDisplay = false, images }: MarkdownContentProps) {
    ...
    return (
        <details className="mt-2">
            ...
            <div className="mt-2 p-3 bg-zinc-700 rounded-sm text-sm">
                <div className="flex justify-between items-center mb-2">
                    <div className="text-xs text-zinc-400">{showRaw ? "Raw Markdown" : "Rendered"}</div>
                    <button onClick={() => setShowRaw(!showRaw)} className="text-xs bg-zinc-600 hover:bg-zinc-500 text-zinc-200 px-2 py-1 rounded-sm transition-colors">
                        {showRaw ? "Show Rendered" : "Show Raw"}
                    </button>
                </div>

                {showRaw ? (
                    <pre className="bg-zinc-800 overflow-x-auto p-3 rounded-sm text-xs text-zinc-300 whitespace-pre-wrap">{displayContent}</pre>
                ) : (
                    <Markdown content={displayContent} images={images ?? {}} />
                )}
